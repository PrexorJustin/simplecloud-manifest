...
case "$OSTYPE" in
   msys*|cygwin*|mingw*)
       ohai "Detected Windows system, downloading Windows installer..."
       curl -o install.tmp.bat https://raw.githubusercontent.com/theSimpleCloud/simplecloud-manifest/main/install.bat
       cmd.exe //c install.tmp.bat
       rm install.tmp.bat
       exit 0
       ;;
esac
...
# Detect platform for Unix systems
detect_platform() {
   OS=$(uname -s)
   ARCH=$(uname -m)
